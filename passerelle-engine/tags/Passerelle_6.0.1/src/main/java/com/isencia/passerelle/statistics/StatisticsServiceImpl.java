...
class StatisticsServiceImpl extends StatisticsServiceDummyImpl implements StatisticsService {
	...
	protected StatisticsServiceImpl() {
		try {
			svr = MBeanServerFactory.createMBeanServer(getServerName());
	...
	public void reset() {
		stop();
		for (Iterator mbNameItr = registeredNames.iterator(); mbNameItr.hasNext();) {
			ObjectName mbName = (ObjectName) mbNameItr.next();
			try {
