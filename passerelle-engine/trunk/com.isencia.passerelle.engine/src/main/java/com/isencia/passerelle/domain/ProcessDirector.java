...
/**
 * Besides the std Ptolemy director stuff, this director adds:
 * <ul>
 * <li> Support for error collectors and error notifications
 * </ul>
 * 
 * @author erwin dl
 */
public abstract class ProcessDirector extends CompositeProcessDirector implements ExecutionControlStrategy {
	 ...
	 */
	private Collection configurableParameters = new HashSet();
     ...
     */
    private Collection firingEventListeners = new HashSet();
     ...
     */
	private Collection errorCollectors = new HashSet();
	
	private ExecutionControlStrategy execCtrlStrategy = new DefaultExecutionControlStrategy();
    ...
    public void notifyFiringEventListeners(FiringEvent event) {
        if(event!=null && event.getDirector().equals(this)) {
            for (Iterator listenerItr = firingEventListeners.iterator(); listenerItr.hasNext();) {
                FiringEventListener listener = (FiringEventListener) listenerItr.next();
    ...
    public void setExecutionControlStrategy(ExecutionControlStrategy execCtrlStrategy) {
    	this.execCtrlStrategy = execCtrlStrategy;
    }
	...
	public void setErrorControlStrategy(ErrorControlStrategy errorCtrlStrategy) {
	...
	public void reportError(PasserelleException e) {
		...
		if(!errorCollectors.isEmpty()) {
			for (Iterator errCollItr = errorCollectors.iterator(); errCollItr.hasNext();) {
				ErrorCollector element = (ErrorCollector) errCollItr.next();
