...
/**
 * The AsyncModelExecutor acts as a Basic Executor that can be used as a Runnable
...
public class AsyncModelExecutor implements Runnable{
    ...
    AsyncModelExecutor(String[] args) {
        this.args = args;
    ...
    public void run() {
        try {
            ...
            if ((file == null) || !file.exists()) {
                logger.error("File "+file.getAbsolutePath()+" not found, execution failed ");
                System.exit(1);
            }

            if (!file.getName().endsWith(".moml") && !file.getName().endsWith(".xml")) {
                logger.error("File "+file.getAbsolutePath()+" is not a moml file, execution failed ");
                System.exit(1);
            }
            ...
            flowManager.executeBlocking(flow, props);
