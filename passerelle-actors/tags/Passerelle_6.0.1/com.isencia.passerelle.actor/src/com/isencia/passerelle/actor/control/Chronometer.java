...
/**
 * 
 * A Chronometer with 2 blocking ports: set and reset.
 * The actor will always first wait for a message on the set port.
 * Then it will look for a message on the reset port.
 * The time interval (in ms) between the two is sent out in the body of 
 * the outgoing message.
 * 
 * If a reset message has already arrived before a set message, the reset
 * will be immediately noticed after the set, and the resulting time interval
 * will be approximately 0.
 * 
 * If another set message arrives before the reset, it means that immediately
 * after consuming the reset, in the next fire() iteration, the actor will be set again.
 * 
 * So, the Chronometer does not drop any messages and it will almost continuously be blocked
 * on one of the 2 input ports.
 * 
 * @author erwin dl
 */
