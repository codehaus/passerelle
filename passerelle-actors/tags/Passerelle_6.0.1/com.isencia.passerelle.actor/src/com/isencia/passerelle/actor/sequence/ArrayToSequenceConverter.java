...
/**
 * This actor expects an array of objects as message payload.
 * It generates a sequence of messages, each containing an individual
 * entry of the incoming array.
 * 
 * If the incoming message does not contain an array, it is sent out
 * unmodified.
 * 
 * @author erwin dl
 */
		...
		try {
			...
			try {
				...
				for(int i=0;i<contentAsArray.length;++i) {
					ManagedMessage elementMsg = MessageFactory.getInstance().createMessageInSequence(seqID,new Long(i),(i >= contentAsArray.length-1),getStandardMessageHeaders());
