 ...
 * <p>
 * A maximal retry count can be configured. Each incoming message is sent out
 * on one of 2 alternative output ports:
 * <ul>
 ...
 * <li>when it has been received more times than the configured retry count,
 * it gets sent out via <code>noRetryOutput</code>.
 * </ul>
 * </p>
 * <p>
 * The retry management is implemented by adding a named counter value as a message header.
 * A message sent out via the <code>retryOutput</code> should be sent back to the <code>input</code>
 * port when a retry is desired. As it will probably first have passed through one or more other
 * actors, care must be taken to ensure that the message's headers do not get lost/corrupted.
 * Otherwise, e.g. if the counter header gets lost, there is a risk for eternal loops!
 * </p>
 * 
 * @author delerw
 *
...
public class RetryLoopActor extends Actor {
	 ...
	 */
	public RetryLoopActor(CompositeEntity container, String name)
