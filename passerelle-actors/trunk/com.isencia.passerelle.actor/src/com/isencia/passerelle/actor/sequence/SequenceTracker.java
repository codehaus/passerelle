 ...
 * <li> when a sequence has completely been handled, the tracker generates 
 * a corresponding notification msg via its <code>seqFinished</code> output
 * port.
 * </ul>
 * 
 * @author erwin dl
...
public class SequenceTracker extends Transformer {
	...
	protected void acceptHandledMessage(ManagedMessage message) {
		...
		if(seqTrace==null) {
			...
			try {
				((Director)getDirector()).reportError(
						new ProcessingException("Received message feedback for unknown sequence "+message.getSequenceID(),message,null));
