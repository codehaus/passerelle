...
public class ActorOrderDialog extends JDialog {
	...
	public ActorOrderDialog(final JFrame owner, final CompositeActor model,
			final HMIDefinition hmiDef) {
		...
		if (model != null) {
			...
			if (hmiDef.getLayoutPrefs(model.getName()) == null
					|| hmiDef.getLayoutPrefs(model.getName()).getActorNames()
							.size() == 0) {
				...
				final List<Parameter> params = model
				...
				final List<String> actorNamesList = hmiDef.getLayoutPrefs(
						model.getName()).getActorNames();
				...
				if (!actorListModel.containsName(ModelUtils
						.getFullNameButWithoutModelName(model, model
								.getDirector()))) {
					actorListModel.add(model.getDirector());
				}
				final List<NamedObj> actors = model.entityList(NamedObj.class);
				for (final NamedObj actor : actors) {
					if (!actorListModel.containsName(ModelUtils
							.getFullNameButWithoutModelName(model, actor))) {
						// System.out.println("adding " + actor.getFullName());
						actorListModel.add(actor);
					}
				}
				...
				for (final Parameter param : params) {
					if (!actorListModel.containsName(ModelUtils
							.getFullNameButWithoutModelName(model, param))) {
						// System.out.println("adding " + param.getFullName());
						actorListModel.add(param);
					}
				}
	...
	class ActorOrderSaver implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			...
			LayoutPreferences layoutPrefs = hmiDef.getLayoutPrefs(model
					.getName());
			if (layoutPrefs == null) {
				layoutPrefs = new LayoutPreferences(1);
				hmiDef.addModelLayout(model.getName(), layoutPrefs);
			}
			layoutPrefs.getActorNames().clear();

			for (final NamedObj actor : actorListModel.getActorList()) {
				layoutPrefs.getActorNames()
						.add(
