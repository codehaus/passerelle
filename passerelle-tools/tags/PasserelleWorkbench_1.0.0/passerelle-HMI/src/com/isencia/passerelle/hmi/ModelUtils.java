...
public class ModelUtils {
     ...
     * <code>name1.name2.name3.name4=value</code>
     * Where <code>name4</code> is the parameter name and all preceeding names are nested actor names
	 * (actors within composite actors within...)
	 * 
	 * @param rootEntity
	 * @param cfgProps
	 * @throws IllegalActionException
	 */
	public static void configure(final CompositeActor rootEntity,
			final Properties cfgProps) throws IllegalActionException {
	 ...
	 */
	public static void configure(final CompositeActor rootEntity,
			final String[] cfgProps) throws IllegalActionException {

		for (final String propDef : cfgProps) {
			final String[] parts = propDef.split("=");
			if (parts.length < 2) {
				throw new IllegalActionException(
						"Invalid parameter override definition " + propDef);
			}
	 ...
	 * @throws IllegalActionException
	 */
	public static Parameter getParameter(final CompositeActor rootEntity,
			final String propName) throws IllegalActionException {
		final String[] parts = propName.split("[\\.=]");
		if (parts.length == 0) {
			throw new IllegalActionException("Invalid parameter name "
					+ propName);
		}
		...
		final Parameter p = (Parameter) e.getAttribute(parts[parts.length - 1],
				Parameter.class);
		return p;
	}

	public static String getFullNameButWithoutModelName(
			final CompositeActor model, final Nameable namedObject) {
		...
		if (namedObject.getName().equals(model.getName())) {
			return "";
		}
		String fullName = namedObject.getFullName();
		if (fullName.length() > model.getName().length() + 1) {
	...
	public static String getFullNameButWithoutModelName(
			final CompositeActor model, String fullName) {
		// we need to obtain the full name of the NamedObj,
		// but without the model name in it
		final int i = fullName.indexOf(model.getName());
		if (i > 0) {
	...
	public static CompositeActor loadModel(final File filePath)
			throws Exception {
		if (filePath == null) {
			return null;
		}
	 ...
	 * @throws Exception
	 */
	public static CompositeActor loadModel(final URL xmlFile) throws IOException, Exception {
		return FlowManager.readMoml(xmlFile);
	}

	/**
     * Tries to locate a file with the given relative or absolute path.
     * <br>
     * If a straight search using java.io.File does not find it,
     * the file is searched for on the classpath using
		...
		if (configuration == null) {
			try {
				final URL cfgURL = MoMLApplication.specToURL(System.getProperty(IPropertyNames.APP_HOME, IPropertyNames.APP_HOME_DEFAULT) + File.separator
						+ System.getProperty(IPropertyNames.APP_CFG, IPropertyNames.APP_CFG_DEFAULT) + File.separator + "runConfiguration.xml");
