...
public final class GenericHMIBean extends JPanel {
	...
	public void setEnable(final boolean enable) {
		if (enable) {
			if (StateMachine.getInstance().getCurrentState().equals(
					StateMachine.READY)
					&& momlLoaded) {
	...
	public void loadSequenceModel(final String sequencePath) {
		if (sequencePath != null && sequencePath.length() != 0) {
			try {
				StateMachine.getInstance().transitionTo(StateMachine.READY);
				mainGenericHMI.getGenericHMI().clearModelForms();
				mainGenericHMI.getGenericHMI().loadModel(new URL(sequencePath),
						null);
				this.removeAll();
				mainPanel = mainGenericHMI.getBeanPanel();
				this.add(BorderLayout.CENTER, mainPanel);
				...
				this.removeAll();
				final JLabel modelNameLabel = new JLabel(sequencePath
				...
				System.out.println(sequencePath
						+ " file not found or is not a moml file");
			}
		}
	}
