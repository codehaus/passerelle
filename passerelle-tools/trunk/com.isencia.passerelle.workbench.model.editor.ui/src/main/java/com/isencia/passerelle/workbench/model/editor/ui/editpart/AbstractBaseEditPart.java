...
abstract public class AbstractBaseEditPart extends
		org.eclipse.gef.editparts.AbstractGraphicalEditPart implements ChangeListener 
		 {
	 ...
	 */
	protected AbstractBaseFigure getComponentFigure() {
		return (AbstractBaseFigure) getFigure();
	}
	...
	public void changeExecuted(ChangeRequest changerequest) {
		...
		if( ActorGeneralSection.class.equals(type)) {
			if( source==this.getModel() && source instanceof NamedObj) {
				...
				if( name != null && !name.equals(getComponentFigure().getName())) {
					...
					getComponentFigure().setName(name);
					getFigure().repaint();
				}
			}
