...
public class ActorGeneralSection extends AbstractPropertySection {
	 ...
	 * 
	 * This method will show an error message to the user if the name is not
	 * legal.
	 * 
	 * @param model
	 * @param text
	...
	public static void setName(final NamedObj model, final String text) {
		...
		if (ModelUtils.isNameLegal(text)) {
			model.requestChange(new ModelChangeRequest(
					ActorGeneralSection.class, model, "changeName") {
	...
	public void createControls(Composite parent, TabbedPropertySheetPage page) {
		...
		layoutData.right = new FormAttachment(nameText,
				-ITabbedPropertyConstants.HSPACE);
		layoutData.top = new FormAttachment(nameText, 0, SWT.CENTER);
		nameLabel.setLayoutData(layoutData);
		
		// Create Type Label Control
		CLabel typeLabel = getWidgetFactory().createCLabel(composite, "Type:");
		layoutData = new FormData();
		layoutData.left = new FormAttachment(0, 0);
		layoutData.top = new FormAttachment(nameLabel, 0);
		typeLabel.setLayoutData(layoutData);
		// Create Type Value Control
		 typeText= getWidgetFactory().createCLabel(composite, "");
		 layoutData = new FormData();
		 layoutData.left = new FormAttachment(nameText,
				 0, SWT.LEFT);
		 layoutData.right = new FormAttachment(nameText,
				 100, 0);
		 layoutData.top = new FormAttachment(nameLabel, 0);
		 typeText.setLayoutData(layoutData);
	}
	...
	public void refresh() {
		...
		if (typeText != null)
			if (model != null) {
				String type = PaletteItemFactory.get()
						.getType(model.getClass());
				if (type != null) {
					typeText.setText(type);
				} else {
					typeText.setText(model.getClassName());
				}
			} else {
