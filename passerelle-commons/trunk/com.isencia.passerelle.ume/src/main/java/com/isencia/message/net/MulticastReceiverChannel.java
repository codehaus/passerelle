...
public class MulticastReceiverChannel extends ReceiverChannel {
	...
	/**
	 * Gets the local port on which this socket is listening
	 * for datagrams.
	 * 
	...
	protected Object doGetMessage() throws ChannelException {
		...
		try {
			...
			throw new ChannelException("IOException while receiving DatagramPacket: "
							+e.getMessage());
		}
	}
	/**
	...
	public void setGroupName(String group) throws ChannelException {
		if(logger.isTraceEnabled())
			logger.trace("setGroupName() - entry :"+group);
	...
	public void setPort(int port) throws ChannelException {
		if(logger.isTraceEnabled())
			logger.trace("setPort() - entry :"+port);
			
		if(port<0) {
			throw new ChannelException(
				"Port ("+port+") not set correctly");
		}
