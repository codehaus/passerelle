...
public class DatagramSenderChannel extends SenderChannel {
	...
	public void setRemotePort(int remotePort) throws ChannelException {
		if(logger.isTraceEnabled())
			logger.trace("setRemotePort() - entry :"+remotePort);

		if(remotePort<0) {
			throw new ChannelException(
				"Remote port ("+remotePort+") not set correctly");
		}
	...
	public void setRemoteHostName(String remoteHost) throws ChannelException{
		if(logger.isTraceEnabled())
			logger.trace("setRemoteHostName() - entry :"+remoteHost);
			
		if(remoteHost==null || remotePort <0) {
			throw new ChannelException(
				"Remote host ("+remoteHost+") not set correctly");
		}
	...
	protected void doSendMessage(Object message) throws Exception {
		if(logger.isTraceEnabled())
			logger.trace("doSendMessage() - entry :"+message);
			
		byte[] bb = message.toString().getBytes();
		DatagramPacket packet = 
			new DatagramPacket(bb,bb.length,getRemoteHost(),getRemotePort());
		socket.send(packet);
