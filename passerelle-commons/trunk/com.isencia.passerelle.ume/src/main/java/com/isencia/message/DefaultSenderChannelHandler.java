...
/**
 * 
 * DefaultSenderChannelHandler
 * 
 * A channel handler implementation that loops over all message providers of the
 * channel, each time it is triggered by a messageAvailable() invocation.
 * 
...
public class DefaultSenderChannelHandler implements ISenderChannelHandler {
	...
	/**
	 * MessageLoop will loop over all providers until they have no more messages available,
	 * for a number of times as specified by the triggercount.
	 * Each finished loop decreases the triggercount.
	 * Each new messageAvailable() notification on SenderChannel increases the triggercount.
	 */
	class MessageLoop extends Thread {
		...
		public void run() {
			...
			try {
				...
				while (foundMsg || !isInterrupted()) {
					foundMsg = false;
					consumeTrigger();
					Iterator providerItr = channel.getProviders().iterator();
					while (providerItr.hasNext()) {
						IMessageProvider provider = (IMessageProvider) providerItr.next();
		...
		/**
		 * Decrements the trigger count. 
		 * If no triggers available, waits for one.
		 */
