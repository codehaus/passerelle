...
public static void main(java.lang.String[] args) {
	...
	try {
		PropertyContainerFactory.instance().registerPropertySource(
			"propertytest.xml");
		
		IPropertyContainer test =
			PropertyContainerFactory.instance().get("be.isencia.properties.test.properties");

		logger.info("Container contains:\n" + test);

		logger.info("Container name: " + test.getName());
		logger.info("Property Parsers.GeneratedEntities.WrapperClass value: "
				+ test.getPropertyValue("Parsers.GeneratedEntities.WrapperClass"));
		logger.info("Property Parsers.GeneratedEntities.Package value: "
				+ test.getPropertyValue("Parsers.GeneratedEntities.Package"));
		logger.info("Property Parsers.GeneratedEntities.BaseClass value: "
				+ test.getPropertyValue("Parsers.GeneratedEntities.BaseClass"));
		...
		java.io.FileOutputStream fo =
			new java.io.FileOutputStream("D:\\TupleTest\\propertytestout.properties");
		props.store(fo,
			"Result of conversion from XmlPropertyContainer to java.util.Properties");
		fo.close();
		...
		java.io.FileInputStream fi =
			new java.io.FileInputStream("D:\\TupleTest\\propertytestout.properties");
		props2.load(fi);
		fi.close();
